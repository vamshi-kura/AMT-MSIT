...
let enableRuleObj = schedule.scheduleJob(enableRule, function () {
  ...
  axios
    ...
    .then((response) => {
      ...
      if (timeDiff <= 60) {
        console.log(
          "The time check has passed",
          api_time.toLocaleString(),
...
ipc.on("userNameResponse", (event, arg) => {
  ...
  if (userName)
    document.getElementById(
      "userGreetings"
    ).innerHTML = `<span id ='greetings'>${greeting}</span>, ${userName}`;
  else
    document.getElementById(
      "userGreetings"
    ).innerHTML = `<span id ='greetings'>${greeting}</span>`;
});
...
ipc.on("attendance-response", (event, arg) => {
  ...
  if (arg && arg.date === login_date_time.toLocaleDateString()) {
    ...
    if (login_date_time - start_att_time <= 0) {
      ...
      login_date_time - start_att_time >= 0 &&
      end_att_time - login_date_time >= 0
    ) {
      document.getElementById("attendance").disabled = false;
  ...
  document.getElementById("get-Date").innerHTML =
    " " +
    login_date_time.toDateString() +
    ", " +
    login_date_time.toLocaleTimeString();
});
...
document.getElementById("attendance").addEventListener("click", () => {
  ...
  axios
    .post(
      "https://hhq5nbplrb.execute-api.ap-south-1.amazonaws.com/v1/attendance/checkin",
      {
        uuid: uuid,
        email: userEmail,
    ...
    .then((response) => {
      document.getElementById("attendance").innerHTML = "Attendance Marked";
      document.getElementById("timestamp").innerHTML =
        marked_date.toDateString() + ", " + marked_date.toLocaleTimeString();
      document.getElementById("attendance").disabled = true;
    ...
    .catch((error) => {
      ...
      alert(
        "Failed to mark attendance, please check your net connection and click login again"
      );
    });
});
...
ipc.on("uuid-response", (event, arg) => {
  ...
  if (arg != null) {
    axios
      .post(
        "https://hhq5nbplrb.execute-api.ap-south-1.amazonaws.com/v1/attendance/checkout",
        {
          UUID: arg,
        }
      )
