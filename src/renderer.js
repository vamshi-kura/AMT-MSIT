...
document.getElementById('Google-sign').addEventListener('click', () => {
  ...
  axios.get('https://hhq5nbplrb.execute-api.ap-south-1.amazonaws.com/v1/login')
    .then((response) => {
...
ipc.on('secondCall', (event, arg) => {
  ...
  axios.post('https://hhq5nbplrb.execute-api.ap-south-1.amazonaws.com/v1/getgoogleresponse', {
    'UUID': arg
  })
    .then((response) => {
      console.log("The res: ", response.data.statusCode)
      if (response.data.body) {
        console.log("Inside the if case in renderer", response.data.body.googleId, response.data.body.profileObj.givenName)
        loginSuccess = true
        ipc.send('secondCallSuccess', {googleID:response.data.body.googleId, userName: response.data.body.profileObj.givenName, email:response.data.body.profileObj.email  })
